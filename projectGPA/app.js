...
// parameter1 是 要控制的對象
// parameter2 是 duration
// parameter3 是 控制對象的 原始狀態
// parameter4 是 控制對象的 動畫結束後
// parameter5 是 提早開始跑
time_line
...
// 1s變高 1.2變寬 0.3s消失 所以式 2500

// 讓整個網站 enter key 都無法使用
...
allSelects.forEach((select) => {
  select.addEventListener("change", (e) => {
    setGPA();
    chandeColor(e.target); //e.target就是select標籤
  });
});
...
function chandeColor(target) {
  if (target.value == "A" || target.value == "A-") {
    target.style.backgroundColor = "#f8ecc9";
    target.style.color = "#353866";
  } else if (
    target.value == "B" ||
    target.value == "B-" ||
    target.value == "B+"
  ) {
    target.style.backgroundColor = "#cff0da";
    target.style.color = "#353866";
  } else if (
    target.value == "C" ||
    target.value == "C-" ||
    ...
    target.style.backgroundColor = "#d3e0f7";
    target.style.color = "#353866";
  } else if (
    target.value == "D" ||
    target.value == "D-" ||
    ...
    target.style.backgroundColor = "#fec8c9";
    target.style.color = "#353866";
  } else if (target.value == "F") {
    target.style.backgroundColor = "#f1404b";
    target.style.color = "#F8FAFF";
  } else {
    target.style.backgroundColor = "#e3dede";
    target.style.color = "#353866";
  }
}
...
addButton.addEventListener("click", () => {
  ...
  newButton.addEventListener("click", (e) => {
    ...
    e.target.parentE1ement.parentElement.addEventListener(
      "animationend",
      (e) => {
        e.target.remove();
        setGPA();
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    ...
    if (
      ...
      let class_object = {
        // 寫法一
        // class_name: class_name, // 設定一個 class_name 的值 要等於 class_name ( 上面的 class_name )
  ...
  // 取得 object array 後，可以把成績 string 換成 number
  // convertor()
  ...
  // ` 為backtick 可把內容轉乘 String，等同於 '' (單引號) 以及 ""(雙引號)

  // 因為 select 無法用 string更改，因此 select 要用JS 直接做更改
